  ...
  return (
    <div className="fixed inset-0 bg-gray-500/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 font-sans text-gray-800">
      <motion.div 
        ...
        className="bg-gray-50 border-2 border-blue-400 p-0 max-w-3xl w-full shadow-2xl flex flex-col"
      >
        <div className="bg-[#e4ebf5] px-4 py-3 border-b border-gray-300 font-bold text-gray-800 tracking-wide text-lg">
          單據狀態轉換設定
        </div>
        
        <div className="p-6 space-y-6 text-lg font-medium bg-white">
          <div className="flex items-center gap-3 border-b border-dashed border-gray-300 pb-6">
            <span>預定單於取餐前</span>
            <select 
        ...
        <div className="bg-[#e4ebf5] px-4 py-3 border-y border-gray-300 font-bold text-gray-800 tracking-wide text-lg">
          顯示設定
        </div>
        
        <div className="p-6 space-y-5 text-lg font-medium bg-white">
          <label className="flex items-center gap-2 cursor-pointer pb-2">
            <input 
              type="checkbox" 
              checked={localSettings.showComboName}
              onChange={(e) => setLocalSettings(s => ({ ...s, showComboName: e.target.checked }))}
              className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 accent-teal-500"
            />
            <span>套餐商品需要顯示套餐名稱</span>
          </label>

          <div className="flex items-center gap-4">
            <span className="w-40">訂單字級</span>
          ...
          <div className="flex items-center gap-4">
            <span className="w-40">資訊刷新頻率</span>
            <div className="flex items-center gap-3">
              <select 
        ...
        <div className="p-6 space-y-4 text-lg font-medium bg-white">
          <div className="flex items-center gap-3">
            <span>訂單進入製餐區後，</span>
            <select 
        ...
        <div className="bg-white p-4 flex justify-end gap-3 border-t border-blue-400">
          <button
            onClick={onClose}
